 ...
 */
public class MainBannerModel<T> extends BaseModel {

    public void getBannerData(Context context, boolean isDialog, boolean cancelable,
                      ObservableTransformer<T,T> transformer, ObserverResponseListener observerListener){

        //当不需要指定是否有dialog时，可以调用这个方法
//        subscribe(context, Api.getApiService().login(map), observerListener);
        subscribe(context, Api.getApiService().getBannerData(), observerListener,transformer,isDialog,cancelable);
    }
